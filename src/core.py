...
def perform_adf_test(series: pd.Series, name: str) -> float:
    ...
    logger.info(f'{name} ADF Statistic: {result[0]:.4f}')
...
def perform_johansen_test(data: pd.DataFrame, det_order: int=0, k_ar_diff: int=1):
    ...
    logger.info('Trace statistics:', johan_test.lr1)
    logger.info('Critical values:', johan_test.cvt)
    return johan_test

def fit_vecm_model(data: pd.DataFrame, k_ar_diff: int=1, coint_rank: int=1, deterministic: str='co'):
    """Fit Vector Error Correction Model."""
    vec_model = VECM(data, k_ar_diff=k_ar_diff, coint_rank=coint_rank, deterministic=deterministic)
...
def compute_rolling_transfer_entropy(df: pd.DataFrame, var1: str, var2: str, window: int=100, bins: int=3, k: int=1) -> tuple[list, list]:

...
def define_tickers_and_time_range() -> None:
    tickers = ['XLK', 'SMH']
...
def load_your_differenced_dataset() -> None:
    df = pd.read_csv('xlk_smh_prices.csv', parse_dates=['Date'], index_col='Date')
...
def load_and_difference() -> None:
    ...
    te_df = pd.DataFrame({'SMH → XLK': te_smh_to_xlk, 'XLK → SMH': te_xlk_to_smh}, index=pd.to_datetime(dates))
...
def use_the_rolling_te_values_as_features() -> None:
...
def load_your_differenced_data() -> None:
    df = pd.read_csv('xlk_smh_prices.csv', parse_dates=['Date'], index_col='Date')
...
def minimalist_style_settings() -> None:
    plt.rcParams.update({'font.family': 'serif', 'axes.spines.top': False, 'axes.spines.right': False, 'axes.spines.left': True, 'axes.spines.bottom': True, 'xtick.direction': 'out', 'ytick.direction': 'out'})
...
def re() -> None:
    # Re-import libraries after notebook state reset
    import matplotlib.pyplot as plt
    import pandas as pd
...
def main() -> None:
    define_tickers_and_time_range()
    plot()
    load_your_differenced_dataset()
    load_and_difference()
    use_the_rolling_te_values_as_features()
    train_test_split()
    notebook_step_008()
    load_your_differenced_data()
    minimalist_style_settings()
    re()
    apply_smoothing()
